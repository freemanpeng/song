...
public class Generate {
	 ...
	 * 随机生成的音符范围
	 */
	private static final byte range = 15;
	/**
	 * 随机选择节奏型
	 */
	private static final byte[] rhythm = Rhythm.rand();
	/**
	 * 随机选择走向
	 */
	private static final byte[] path = Path.rand();

	/**
	 * 速度
	 */
	private static final int bpm = 118;
	/**
	 * 96代表半拍，192是一拍
	 */
	 ...
	 * 上一个音符，初始化设置为9
	 */
	private static int prev = 9;

	public static void main(String... args) throws Exception {
		File file = new File("./generate.mid");
		...
		do {
			for (int i = 0, chd = 0; i < rhythm.length; i++) {
				// 计算音符时间
				int pos = velocity * ((cur * rhythm.length + i) + 2);
